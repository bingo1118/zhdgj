...
public class UserOfParentAreaAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public UserOfParentAreaAdapter(Context mContext, List<AccountEntity> listNormalSmoke,Area area) {
    ...
    private void setRole(final AccountEntity entity) {
        if(!(MyApp.entity.getGrade()<1||isManegerOfArea(MyApp.entity.getUserId()))){
    ...
    private void unbindArea(final Area entity,final AccountEntity mAccount) {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
        builder.setTitle("警告");
        builder.setMessage("确定将该账号移出该一级区域以及下属二级区域?");
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                String url = ConstantValues.SERVER_IP_NEW + "unbindParentArea?userId=" + mAccount.getUserId()
