package com.smart.cloud.fire.view.dateSelector;
...
public class DateTimePicker extends WheelPicker {
    ...
    public DateTimePicker(Activity activity, @DateMode int dateMode, @TimeMode int timeMode) {
        ...
        int hour = t.hour;
        int minute=t.minute;
        setSelectedItem(year,month+1,date,hour,minute);
    }
...
//                layout.setWeightSum(5);
    ...
    private void initHourData() {
        ...
        for (int i = startMinute; i <= endMinute; i += stepMinute) {
            String minute = DateUtils.fillZero(i);
            minutes.add(minute);
        }
        if (hours.indexOf(selectedMinute) == -1) {
            //当前设置的小时不在指定范围，则默认选中范围开始的小时
